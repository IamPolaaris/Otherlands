...
{
	internal class MoltenGodHead : MoltenGod
	{
		...
		public override void CustomBehavior() {
			if (Main.netMode != NetmodeID.MultiplayerClient) {
				...
				if (attackCounter <= 0 && Vector2.Distance(npc.Center, target.Center) < 200 && Collision.CanHit(npc.Center, 1, 1, target.Center, 1, 1)) {
					...
					int projectile = Projectile.NewProjectile(npc.Center, direction * 5, ProjectileID.EyeLaser, 45, 0, Main.myPlayer);
					Main.projectile[projectile].timeLeft = 300000;
					attackCounter = 25;
					npc.netUpdate = true;
				}
			}
	...
	internal class MoltenGodBody : MoltenGod
	{
	...
	internal class MoltenGodTail : MoltenGod
	{
	...
	public abstract class MoltenGod : Worm
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Molten God, King of hell");
		}

		public override void Init() {
			minLength = 53;
			maxLength = 53;
			tailType = ModContent.NPCType<MoltenGodTail>();
			bodyType = ModContent.NPCType<MoltenGodBody>();
			headType = ModContent.NPCType<MoltenGodHead>();
			speed = 12.3f;
			turnSpeed = 0.350f;
		}
	}
	...
	public abstract class Worm : ModNPC
	{
		...
		public override void AI() {
			...
			if (npc.ai[1] > 0f && npc.ai[1] < (float)Main.npc.Length) {
				...
				npc.position.Y = npc.position.Y + num192; //ShadowBeamHostile num188
				if (directional) {
					if (num191 < 0f) {
						npc.spriteDirection = 1;
					}
		...
		public override bool? DrawHealthBar(byte hbPosition, ref float scale, ref Vector2 position) {
			return head ? (bool?)null : false;
						 ...
						 {
							 Item.NewItem(npc.getRect(), ModContent.ItemType<FleshBar>(), 30);
						 
						 	 if (Main.rand.Next(8) == 1) 
							 Item.NewItem(npc.getRect(), ModContent.ItemType<FleshWaders>(), 1);
						 }
	}
}